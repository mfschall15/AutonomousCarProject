...
class VirtualDriver():
  ...
  def drive(self):
    ...
    self.__image = cv2.resize(cv2.cvtColor(step[0], cv2.COLOR_RGB2BGR), (256, 128))
  ...
  def __mouse_callback(self, event, x, y, flags, param):
    ...
    if self.__driving:
      self.__x =  2.0 * (int(x) / 256.0 - 0.5)
      self.__y = -2.0 * (int(y) / 128.0 - 0.8)
      ...
      if (time.time() - self.__sample_time > self.__fps):
        ...
        # img_name = str(self.__x) + "_" + str(self.__y) + "_" + str(now) + ".jpg"
        img_name =  str(now) + "_" + str(self.__x) + "_" + str(self.__y) + "_" + ".jpg"
        cv2.imwrite(self.__image_dir + "/" + img_name, self.__image)
    return
...
if __name__ == '__main__':
  ...
  config = {
    'car': {
      ...
      "img_w"     : 256,                   # The image width
      "img_h"     : 128,                   # The image height
